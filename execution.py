...
def async_task_callback(prompt_id):
    try:
        gpu_uuid = task_callback(prompt_id, "TASK_RUNNING", None, None)
        logging.info(f"Task Callback completed, prompt_id: {prompt_id}, status: TASK_RUNNING, gpu_uuid: {gpu_uuid}")
    except Exception as e:
